...
saveRDS(PAS_removed,'../DATA//PAS_removed.rds')
saveRDS(Norm_removed,'../DATA/Norm_removed.rds')
...
# load dataset
LoadData("ifnb")
PAS_removed = readRDS('../DATA/PAS_removed.rds')
Norm_removed = readRDS('../DATA/Norm_removed.rds')
...
p1 + p2
ggsave('../figure/umap_combine_remove.png',width = 10, height = 5)
...
# data in "RNA" slot, and find degs in combined sample between celltypes
setwd('/Users/liyaqi/Dropbox/Single_cell/placenta/script/')
immune.combined = readRDS('../DATA/combined_remove.rds')
...
write.csv(nk.markers,'../figure/VSMC_6_deg.csv')
...
write.csv(VSMC_12.markers,'../figure/VSMC_12_deg.csv')
...
write.csv(EVT_15.markers,'../figure/EVT_15_deg.csv')
...
```{r}
##在combined cells中找celltype的marker
...
plot(p)
ggsave('../figure/check_marker_gene.png',width = 20,height = 15)

DimPlot(object =immune.combined,  group.by='SingleR_celltype')
ggsave('../figure/SingleR_in_combined.png',width = 20,height = 15)
```

##Auto
```{r}

##使用标准ref数据库，SingleR主注释
...
ggsave('../figure/SingleR_combine.png',width = 15,height = 10)
##使用HCA ref注释，见test.R
```
...
set.seed(123)
plot_test(PAS_removed,'Graph.2.cell','PAS',Norm_removed,'draft','Control')
plot_test(subset(immune.combined, subset = orig.ident=='PAS'),'SingleR_celltype','PAS',subset(immune.combined, subset = orig.ident=='Norm'),'SingleR_celltype','Control')
##这个此前只是用来看PAS_removed Control_removed 手动注释的两列的细胞类型的柱状图，但是合并后SingleR自动注释的，在一个combined 的Seurat obj里，还没有被
ggsave('../figure/SingleR_celltype_compare.png')
```
...
DEGs_celltype=function(celltype){
  immune.combined.sct = immune.combined[,immune.combined@meta.data[,'SingleR_celltype']==celltype]
  ...
  immune.combined.sct.markers %>%
    group_by(cluster) %>%
    top_n(n = 10, wt = avg_log2FC) -> top10
  title= paste("TOP10 DEGs in ",celltype," between PAS and Norm sample")
  DoHeatmap(immune.combined.sct, features = top10$gene) +ggtitle(title)

  my_file = paste('./local_figure/DEGs_',celltype,'_between_two_sample.png',sep='')
  ggsave(my_file,width = 10, height = 10)
 ...
 avg_exp <- data.frame(avg_exp$integrated)
...
# Create the scatter plot
 ggplot(com_exp_long, aes(x = cluster.x, y = gene.x, color = z_score,size=percent)) +
  ...
  scale_color_gradient2(low = muted("blue"), mid = "white", high = muted("red"),midpoint =( max(com_exp_long$z_score)+min(com_exp_long$z_score))/2) +
  theme_bw() +
  ...
  labs(x = "Cluster",
   ...
   ggtitle(title)
  my_file = paste('./local_figure/top10_DEGs_ave_',celltype,'_two_sample.png',sep='')
  ggsave(my_file,width = 10, height = 10)

}
print("chose cell type")
unique(immune.combined@meta.data$SingleR_celltype)
DEGs_celltype('syncytiotrophoblast cell')
DEGs_celltype('Hofbauer cell')
DEGs_celltype('extravillous trophoblast cell')
DEGs_celltype('decidual natural killer cell')
DEGs_celltype('placental villous trophoblast')
DEGs_celltype('macrophage')
DEGs_celltype('endothelial cell')
DEGs_celltype('CD16-positive, CD56-dim natural killer cell')
DEGs_celltype('fibroblast')
DEGs_celltype('decidual cell')
##这个不能看，norm的太少了
DEGs_celltype('glandular epithelial cell')
